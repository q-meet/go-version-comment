...
type FD struct {
	...
	// I/O poller.
	// 这是 Go 对 poll过程的一个抽象，所有平台的抽象都是一样的
	pd pollDesc
	...
	// 区分这个是不是一个流式的 FD, 与流式相反的是基于 packet的，即 UDP socket。 该值不可变
	IsStream bool

	// Whether a zero byte read indicates EOF. This is false for a
	// message based socket connection.
	// 当连接读到 0长度时，用来区分是否代表 EOF。 如果是基于消息的 socket 连接始终是false
	ZeroReadIsEOF bool
